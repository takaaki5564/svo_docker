#!/usr/bin/env python3
import time, sys, os
from ros import rosbag
import roslib, rospy
roslib.load_manifest('sensor_msgs')
from sensor_msgs.msg import Image

from cv_bridge import CvBridge
import cv2

#TOPIC = 'camera/image_raw/compressed'
TOPIC = '/camera/image_raw'

def CreateVideoBag(videopath, bagname, width):
    '''Creates a bag file with a video file'''
    bag = rosbag.Bag(bagname, 'w')
    cap = cv2.VideoCapture(videopath)
    cb = CvBridge()
    prop_fps = cap.get(cv2.CAP_PROP_FPS)
    if prop_fps != prop_fps or prop_fps <= 1e-2:
        print ("Warning: can't get FPS. Assuming 24.")
        prop_fps = 24
    print ("Set fps: ", prop_fps)

    ret, frame = cap.read()
    if not ret:
        print("Failed to capture frame")
        exit()

    h, w = frame.shape[:2]
    width = int(width)
    height = round(h * (width / w))
    print("Set frame width={} height={}".format(width, height))

    ret = True
    frame_id = 0
    while(ret):
        ret, frame = cap.read()
        if not ret:
            break

        frame = cv2.resize(frame, dsize=(width, height))
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        frame = cv2.equalizeHist(frame)
        frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)

        stamp = rospy.rostime.Time.from_sec(float(frame_id) / prop_fps)
        #stamp = rospy.Time.from_sec(time.time())

        frame_id += 1
        #image = cb.cv2_to_compressed_imgmsg(frame)
        image = cb.cv2_to_imgmsg(frame, encoding='bgr8')
        image.header.stamp = stamp
        image.header.frame_id = "camera"
        bag.write(TOPIC, image, stamp)
    cap.release()
    bag.close()
    print("Finished n={} frames".format(frame_id))


if __name__ == "__main__":
    if len( sys.argv ) == 4:
        CreateVideoBag(*sys.argv[1:])
    else:
        print( "Usage: video2bag videofilename bagfilename width")
